  ...
  if (address < PHYS_BASE) 
    {
	  ...
	  /* ADDED THIS */
      /* No page found. Check to see if it's on the stack and there's room for it.*/
...
page_out (struct page *p) 
{
  ...
  /* ADDED THIS */
  /* Check whether the frame is dirty*/
	dirty = pagedir_is_dirty (p->thread->pagedir, (const void *) p->addr);

  /* ADDED THIS */
  /* Write frame contents to disk if necessary. */
  /* Check if frame needs to be written. */
	if(p->file == NULL) { // No file, so doesn't need to be written
		ok = swap_out(p); 
	}
	else if (dirty){ // There is a file and it's dirty.
		if(p->private) { // Private file. 
			ok = swap_out(p); 
		}
		else { //Dirty and not private. Write out. 
			ok = file_write_at(p->file, (const void *) p->frame->base, p->file_bytes, p->file_offset);
		}
	}

  /* ADDED THIS */
  /* if the frame was evicted correctly, remove the frame */
	if(ok) {
		p->frame = NULL;
	}
  return ok;
